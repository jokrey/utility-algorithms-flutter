...
class ObservableVCall1to1Widget extends StatefulWidget {
  final ObservableVCall1to1 _call;

  ///Constructor
  ObservableVCall1to1Widget({Key key, @required VCall1to1 call})
      : _call = call, super(key: key);

  @override
  _ObservableVCall1to1WidgetState createState() =>
      _ObservableVCall1to1WidgetState(_call);
}

class _ObservableVCall1to1WidgetState extends State<ObservableVCall1to1Widget> {
  final RTCVideoRenderer _localRenderer = RTCVideoRenderer();
  final RTCVideoRenderer _remoteRenderer = RTCVideoRenderer();
  final ObservableVCall1to1 call;

  _ObservableVCall1to1WidgetState(this.call) {
    ...
    _remoteRenderer.onResize = () {
      if(call.signaler != null && call.signaler.isConnected()) {
        setState(() {});
      }
    };
    call.remoteObserverProvider.addObserver((stream) async {
      if(mounted && call.signaler != null && call.signaler.isConnected()) {
        setState(() {});
      }
    });
            ...
            child: Stack(children: <Widget>[
              ...
              Positioned(
                ...
                child: call.remoteObserverProvider.isConnected()?
                  Icon(Icons.person_pin_outlined) : Text("")
              ),
            ]),
          );
        })
