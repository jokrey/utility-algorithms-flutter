...
class VCall1to1RemoteObserver {
  ...
  VCall1to1RemoteObserver(List<String> remoteNames, this.signaler, iceServers) {
    for (var remoteName in remoteNames) {
      remoteProviders
          .add(RemoteVideoProviderInternal.createWith(remoteName, iceServers));
    }
  ...
  void _sharedConstructorInit() {
    for (var remoteProvider in remoteProviders) {
      ...
      remoteProvider
          .setTransceiverSpecificationCallback((peerConnection) async {
