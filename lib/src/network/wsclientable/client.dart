...
class ClientConnection {
  ...
  final Map<String, Function(String, ClientConnection, Map<String, dynamic>)> _messageHandlers = {};
  ...
  ClientConnection(this._conn) {
    ...
    onDone: () {_remoteClosed();});
    _isConnected = true;
  }

  ///Adds a message handler to this client connection
  // ignore: lines_longer_than_80_chars
  void addMessageHandler(String mType, Function(String, ClientConnection, Map<String, dynamic>) messageHandler) {
...
Future<void> addCertificateFromAssets(String assetURI) async {
  try {
    await rootBundle.load(assetURI).then((data) {
      ...
      print("Added certificate: ${Utf8Decoder().convert(
